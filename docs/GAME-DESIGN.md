...
**Desktop** : WASD déplacer · souris viser · clic gauche attaquer (maintenir = tir continu) · clic droit ou Espace super · **G fumigène à ses pieds** (le bouton ☁ le place à la souris) · Q tir auto-visé · Échap pause.
...
- **Bouton ☁ fumigène**, juste à gauche du super, plus petit — tap = fumigène **à ses pieds**, pour décrocher · glisser dessus = choisir l'endroit puis relâcher. Le badge indique les grenades restantes, l'anneau bleu se remplit vers le prochain jet.
...
- **Grenade fumigène** : le seul gadget, identique pour les dix brawlers, et **un objet à trouver, pas un équipement**. Tu commences chaque vie **les mains vides** ; les grenades sortent des caisses, **une sur cinq**, et tu n'en portes que **2** à la fois. **Rien ne se recharge tout seul** : quand tu as jeté la dernière, il faut recasser des caisses. À la mort, ce qui reste dans la poche retombe au sol comme les cubes. À 90 caisses pour 20 joueurs, il y a moins d'une grenade par joueur dans tout le stock, et les tests le vérifient sur les cinq modes. Portée **9 blocs**, **0,55 s** de vol, puis un nuage de **5 blocs de rayon** qui monte en **0,45 s**, tient **7 s** et se dissipe sur la dernière **1,2 s**. **3 s** de verrou entre deux jets.
  **Deux usages, deux entrées.** L'entrée rapide — `G` au clavier, tap sur le bouton ☁ au doigt — lâche la grenade **à ses propres pieds** : le nuage se referme sur toi, celui qui te tirait dessus te perd, tu sors. C'est la fuite, et elle est garantie. L'entrée délibérée — clic sur le bouton ☁ à la souris, glisser dessus au doigt — pose le nuage là où tu vises, pour couper un couloir sans te cacher toi-même. `smokeCoversThrower()` dit lequel des deux tu viens de faire.
  **Les 4 premières secondes t'appartiennent.** Un nuage frais est **à sens unique** : l'équipe qui l'a lancé voit au travers, personne ne voit à l'intérieur. La fenêtre court à partir de la taille pleine et vaut pour l'équipe entière — en Duo et Trio ton coéquipier en profite aussi. Passé ce délai le nuage aveugle tout le monde pareil, toi compris. À l'écran, ton propre nuage est rendu presque transparent tant que la fenêtre est ouverte (0,22 d'opacité contre 0,88), puis s'épaissit en une demi-seconde. Le badge au-dessus du bouton ☁ affiche « HIDDEN · SEEING 4s », puis « HIDDEN · BLIND », et « SPOTTED · THEIR SMOKE » en rouge quand c'est leur nuage frais qui t'entoure. Toute cette lecture sort de `smokeCover()`, dans le cœur, donc testée.
  **Ce que la fumée ne fait jamais.** Elle **ne bloque aucun projectile**. À moins de **2 blocs** elle ne cache plus personne. Le cercle blanc au sol est exactement le disque qui aveugle. Les bots s'en servent en fuite avec les mêmes règles. Tout est dans `WBCore` (`SMOKE`, `BOX_DROP`, `smokeRadius`, `segmentHitsDisc`, `smokeSeeThrough`, `smokeCover`, `smokeSightBlocked`) ; `SMOKE_READY` retire le gadget d'un build d'un seul booléen.
