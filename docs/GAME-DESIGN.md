...
**Desktop** : WASD déplacer · souris viser · clic gauche attaquer (maintenir = tir continu) · clic droit ou Espace super · **G** appui bref = fumigène à ses pieds, maintenu = on vise et il part au relâchement · Q tir auto-visé · Échap pause.
...
- **Bouton grenade**, juste à gauche du super, plus petit — tap = fumigène **à ses pieds**, pour décrocher · glisser dessus = choisir l'endroit puis relâcher. Le badge indique les grenades restantes, l'anneau bleu se remplit vers le prochain jet.
...
- **Caisses** (90 par map, 90 PV) : casser → **50 % un cube de pouvoir ⬢**, **30 % un cœur ♥**, **20 % une grenade fumigène ☁**. Une seule table, `BOX_DROP` + `boxDrop()` dans le cœur, dont les trois parts font toujours 1. Le cube donne +10 % PV max et +10 % dégâts (max 10) ; le cœur rend **40 % des PV max instantanément**, soit environ 5 secondes de régénération économisées. Un cœur laissé au sol si on est déjà au maximum.
...
- **Grenade fumigène** : le seul gadget, identique pour les dix brawlers, et **un objet à trouver, pas un équipement**. Tu commences chaque vie **les mains vides** ; les grenades sortent des caisses, **une sur cinq**, et tu n'en portes que **2** à la fois. **Rien ne se recharge tout seul** : quand tu as jeté la dernière, il faut recasser des caisses. À la mort, ce qui reste dans la poche retombe au sol comme les cubes. À 90 caisses pour 20 joueurs, il y a moins d'une grenade par joueur dans tout le stock, et les tests le vérifient sur les cinq modes. Portée **9 blocs**, **0,55 s** de vol, puis un nuage de **5 blocs de rayon** qui monte en **0,45 s**, tient **7 s** et se dissipe sur la dernière **1,2 s**. **3 s** de verrou entre deux jets.
  **Deux usages, deux entrées.** L'entrée rapide — appui bref sur `G`, tap sur le bouton grenade au doigt — lâche la grenade **à ses propres pieds** : le nuage se referme sur toi, celui qui te tirait dessus te perd, tu sors. C'est la fuite, et elle est garantie. L'entrée délibérée — **maintenir `G`**, viser, relâcher ; ou glisser sur le bouton au doigt — pose le nuage là où tu vises, pour couper un couloir sans te cacher toi-même. Le seuil est `SMOKE.tapMax` (0,22 s) et la règle vit dans le cœur, `smokeHoldAim()` / `smokeHoldDist()`, pour que le clavier et le doigt la partagent. L'anneau d'impact apparaît dès que l'appui dépasse le seuil : on voit qu'on est en train de viser. `smokeCoversThrower()` dit lequel des deux tu viens de faire.
  **Les 4 premières secondes t'appartiennent.** Un nuage frais est **à sens unique** : l'équipe qui l'a lancé voit au travers, personne ne voit à l'intérieur. La fenêtre court à partir de la taille pleine et vaut pour l'équipe entière — en Duo et Trio ton coéquipier en profite aussi. Passé ce délai le nuage aveugle tout le monde pareil, toi compris. À l'écran, ton propre nuage est rendu presque transparent tant que la fenêtre est ouverte (0,22 d'opacité contre 0,88), puis s'épaissit en une demi-seconde. Le badge au-dessus du bouton grenade affiche « HIDDEN · SEEING 4s », puis « HIDDEN · BLIND », et « SPOTTED · THEIR SMOKE » en rouge quand c'est leur nuage frais qui t'entoure. Toute cette lecture sort de `smokeCover()`, dans le cœur, donc testée.
  **L'objet.** Modèle d'après la fiche validée : corps olive `0x5A6B52` r 0,30 sur 0,86 en 14 segments, deux bandes claires `0xEDF2E8` r 0,315 aux hauteurs 0,30 et 0,70, capuchon tronqué `0x3E4A39` 0,20 → 0,26, levier `0xB9C2AE` incliné de 0,22 rad, goupille en tore `0xC9D2BE` r 0,11, grille sombre sur le dessus, contour BackSide à 1,10. L'ensemble à **0,65**, soit **0,73 unité de haut**. La fiche donnait 0,44 : à cette taille l'objet ne faisait qu'une quinzaine de pixels et son dessin ne survivait pas, les bandes claires et le contour mangeaient l'olive. À 0,65 il reste le plus petit élément du décor (souche 0,80 · caisse 0,82 · botte 0,92 · tonneau 0,98 · brawler 1,44) et se lit au premier coup d'œil. `makeNade()` construit aussi bien celui qui est posé au sol que celui qui vole : un seul constructeur, pour qu'ils ne divergent jamais. L'icône du HUD reprend les mêmes teintes et les mêmes proportions.
  **Ce que la fumée ne fait jamais.** Elle **ne bloque aucun projectile**. À moins de **2 blocs** elle ne cache plus personne. Le cercle blanc au sol est exactement le disque qui aveugle. **Pris dans un nuage qui ne te sert pas** — celui d'un adversaire, ou le tien une fois la fenêtre fermée — l'écran entier se charge de blanc et la carte devient à peine lisible jusqu'à ce que tu en sortes. Un peu moins dense juste autour de toi, pour garder tes propres pieds. C'est `smokeCover()` qui décide, la même fonction que le badge. Les bots s'en servent en fuite avec les mêmes règles. Tout est dans `WBCore` (`SMOKE`, `BOX_DROP`, `smokeRadius`, `segmentHitsDisc`, `smokeSeeThrough`, `smokeCover`, `smokeSightBlocked`) ; `SMOKE_READY` retire le gadget d'un build d'un seul booléen.
